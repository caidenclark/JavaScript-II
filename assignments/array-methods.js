...
ticketPriceTotal = runners.reduce((currentValue, item) => currentValue + item.donation, 0);
...
runners.forEach(item => company.push(`${item.company_name}: ${item.donation}`));

console.log(company);

// Problem 2 - Emails with corresponding runner ID
let email = [];
runners.forEach(item => email.push(`${item.id}, ${item.email}`));

console.log(email);

// Problem 3 - Everybody who donated over 150 dollars
let topDonation = [];
topDonation = runners.filter(item => item.donation > 150);
